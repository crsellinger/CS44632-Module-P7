...
def ingest_sales_data_from_dw() -> pd.DataFrame:
    ...
    try:
        ...
        sales_df = pd.read_sql_query(
            "SELECT * FROM (SELECT * FROM sale INNER JOIN customer ON customer.customer_id = sale.customer_id) AS t INNER JOIN product ON product.product_id = t.product_id;",
            conn,
        )
        conn.close()
        logger.info("Sales data successfully loaded from SQLite data warehouse.")
        
        # Rename second duplicate product_id column to not get KeyError in create function
        sales_df.columns.values[15] = "product.product_id"
